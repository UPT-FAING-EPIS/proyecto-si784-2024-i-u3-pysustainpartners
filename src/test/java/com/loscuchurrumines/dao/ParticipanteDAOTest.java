...
public class ParticipanteDAOTest {
    ...
    public void testCrearParticipanteConFkRolNo1() throws Exception {
        ...
        doReturn(true).when(spyParticipanteDAO).insertarMetodo(anyString(), eq(participante));

        boolean result = spyParticipanteDAO.crearParticipante(participante, 100);

        assertTrue(result);
        verify(spyParticipanteDAO, times(1)).insertarMetodo(anyString(), eq(participante));
    }
    ...
    public void testInsertarMetodoFalla() throws Exception {
        ...
        when(mockConnection.prepareStatement(query)).thenReturn(mockStatement);
        when(mockStatement.executeUpdate()).thenThrow(new RuntimeException("Database error"));
